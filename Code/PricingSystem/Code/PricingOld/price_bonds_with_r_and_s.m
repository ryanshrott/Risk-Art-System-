...
T = (cellfun(@datenum, raw_data(:, find(strcmp(column_labels, 'Maturity'...
    )))) - repmat(today, num_of_bonds, 1))/Constants.dpa;
...
% Excel file.
CAD_rates = CAD_rates(end, 2:end);
% EURO rates
[EUR_rates, ~, ~] = xlsread(file_path, 'EUR');
% Keep the most recent rates. They are contained in the the last row of the
% Excel file.
EUR_rates = EUR_rates(end, 2:end);
% USD rates
[USD_rates, ~, ~] = xlsread(file_path, 'USD');
% Keep the most recent rates. They are contained in the the last row of the
% Excel file.
USD_rates = USD_rates(end, 2:end);
...
for k = 1:size(bond_durations, 1)
    ...
    switch currency{k}
        ...
        case 'EUR'
            ...
            exchange_rate = SpotFXrates.EURCAD;
        case 'USD'
            % fprintf('Will use USD rates\n')
            zero_rates = USD_rates;
            exchange_rate = SpotFXrates.USDCAD;
    end
    ...
    bond_prices(k) = exchange_rate*sum(c.*exp(-y(k)*t));
    bond_durations(k) = sum(c.*t.*exp(-y(k)*t))/bond_prices(k);
    bond_market_prices(k) = exchange_rate*bond_market_prices(k);
...
set(gca, 'FontSize', double(Stylise.font_size))
figure
plot(abs(bond_prices - bond_market_prices), 'LineWidth', Stylise.line_width)
title('Absolute difference between bond prices and bond market prices')
% ]]] DEV MODE

abs_bond_price_error = abs(bond_prices_r_s - bond_market_prices);
