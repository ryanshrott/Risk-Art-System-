function [portfolio_value, bonds_value, options_value, stocks_value, cds_values] = price(x)
% ------------------------------------------------------------------------
...
if bonds == 1
    ...
    bonds_value = price_bonds_with_r_and_s_implied(x, c);
...
if stocks == 1
    ...
    stocks_value = x.USDCAD.*x.stock_prices.*c.stock_positions;
...
%% Return
portfolio_value = sum(bonds_value) + sum(options_value) + sum(stocks_value) + sum(cds_values);
