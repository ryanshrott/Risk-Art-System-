...
spread = x.historical_implied_spreads;
%ratings = x.cds_underlying_rating;

lambdaTimes = repmat(c.CDS_lambda_times, num_of_cds, 1);

zeroTermTimes = repmat(c.CDS_time_structure, num_of_cds, 1);

lambda = x.CDS_lambdas;
%lambda = sortrows(lambda);
...
for k = 1:num_of_cds
    ...
    pSurvivalToPeriodEnd = exp(-repmat(interp(T, lambdaTimes(k, :), lambda((k), :)),1,length(paymentTimes)).*paymentTimes);
    
    pDefaultDuringPeriod = exp(-repmat(interp(T, lambdaTimes(k, :), lambda((k), :)), 1, length(defaultTimes)).*(defaultTimes-deltaDefault/2))...
                           - exp(-repmat(interp(T, lambdaTimes(k, :), lambda((k), :)), 1, length(defaultTimes)).*(defaultTimes+deltaDefault/2));
