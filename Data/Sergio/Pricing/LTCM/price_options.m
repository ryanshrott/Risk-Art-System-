...
eur = 1:1:2;
amer = 3;
[~, option_prices(eur)] = blsprice(S0(eur)*(1-0.1821), K(eur), r(eur)*(1-0.1278), T(eur),...
    vols(eur)*2.1333, y(eur));
[~, tmp] = binprice(S0(amer)*(1-0.1821), K(amer), r(amer)*(1-0.1278), T(amer), 0.1,...
    vols(amer)*2.1333, 1, y(amer));
option_prices(amer) = tmp(1);
% mkt_option_prices = values_of('Last Price');

options_value = option_prices'*values_of('Position')*usdcad*(1.02338);
