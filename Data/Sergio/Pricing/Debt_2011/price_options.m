...
eur = 1:1:2;
amer = 3;
[~, option_prices(eur)] = blsprice(S0(eur)*(1-0.1677), K(eur), r(eur)*(1-0.19), T(eur),...
    vols(eur)*1.9044, y(eur));
[~, tmp] = binprice(S0(amer)*(1-0.1677), K(amer), r(amer)*(1-0.19), T(amer), 0.1,...
    vols(amer)*1.9044, 1, y(amer));
option_prices(amer) = tmp(1);
% mkt_option_prices = values_of('Last Price');

options_value = option_prices'*values_of('Position')*usdcad;
